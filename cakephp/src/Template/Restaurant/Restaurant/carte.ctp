<div id="carte" class="carte container-fluid">
    ...
    <?php foreach ($produits as $produit) : ?>
        ...
        <?php if (isset($produit->r_carte_s_category) && $produit->r_carte_s_category->idRCarteSCategorie != $scategorie) : ?>
                        <div class="row carte-menu col-lg-6 col-md-6 col-sm-12">
                            <h3 class="menu-title"><?= $produit->r_carte_s_category->nomRCarteSCategorie ?></h3>
            <?php $scategorie = $produit->r_carte_s_category->idRCarteSCategorie ?>
        <?php endif; ?>

                            <p class="item-text col-lg-10 col-md-8 col-sm-8 col-xs-8"><?= $produit->nomRCarteProduit ?></p>
                        <?php if ($produit->prixRCarteProduit != 0) : ?>
                            <p class="text-right col-lg-2 col-md-4 col-sm-4 col-xs-4"><?= $produit->prixRCarteProduit ?> €</p>
                        <?php endif; ?>
    <?php endforeach; ?>
